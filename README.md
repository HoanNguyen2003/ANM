# ANM# WebHawk/Catch 2.0

WebHawk/Catch 2.0 is an unsupervised machine learning tool for detecting intrusions/suspicious activities in HTTP logs.
...
The latest version (v3) includes:

- **Fixed critical issues**:

  - Resolved invalid escape sequences in logging statements (changed `\!` to `\\!`)
  - Fixed regex pattern for Apache logs to properly match all log entries
  - Corrected syntax errors in utilities.py (fixed improperly joined lines)
  - Properly handled the string column 'log_line' that couldn't be converted to float for PCA analysis

- **Improvements**:
  - Enhanced error handling and logging
  - Better handling of non-numeric data
  - More robust processing of dataframes
  - Added support for specifying the number of PCA components
  - Improved documentation and code organization

## How It Works

1. **Data Collection**: Reads and parses the specified log file.
2. **Feature Extraction**: Extracts relevant features from the log entries.
3. **Data Preprocessing**: Handles non-numeric data and standardizes features if requested.
4. **Dimensionality Reduction**: Applies PCA to reduce the dataset to 2D (or a specified number of dimensions).
5. **Clustering**: Uses DBSCAN to identify clusters and anomalies in the data.
6. **Anomaly Detection**: Points labeled as noise (-1) by DBSCAN are considered high-severity findings.
7. **Minority Cluster Detection**: Small clusters are considered medium-severity findings.
8. **Visualization**: Creates plots of the detected anomalies if requested.
9. **Reporting**: Generates an HTML report if requested.
